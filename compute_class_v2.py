...
#to obtain decimal number from the stdin device keyboard
    ...
    # Normally Ctrl+D for exit from the stdin keyboard
            ...
            if len(inputs)+1 > 100: # there are up to 100 lines numbers for the standard input
                ...
                #with consideration of the range of input decimal numbers
                temp=float(line)
...
#handling the inputs in terms of requests from the problem
    def analyze_inputs(self,inputs):
        ...
        for num in inputs:
            if num <= self.threshold:  #in case of the input number is less than or equal to the threshold
                outputs.append(0.0)
            elif num <= self.limit:    #in case of the input number is less than or equal to the limit
                if reachLimit:
                    outputs.append(0.0)
                else: 
                    if(self.limit-sum > num): 
                        outputs.append(num-self.threshold)
                    else: 
                        outputs.append(self.limit-sum)
...
#disply results on screen
    def print_outputs(self,outpus,sum):
        print()
        for num in outputs:
            print(num)
        print(sum)
...
#with consideration of all decimal numbers between 0.0 and 10,000,000.0 (inclusive)
#The standard input or stdin device to give commands is the keyboard
    analyzer=Compute(threshold,limit)
    inputs=analyzer.read_inputs()

#begin to analyze inputs
    outputs,sum=analyzer.analyze_inputs(inputs)

#The standard output or stdout device is the terminal screen
    analyzer.print_outputs(outputs,sum)
